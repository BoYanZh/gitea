...
const sfc = {
  ...
  data: () => ({
    ...
    // * login: (...)
    // * name: (...)
    activityTopAuthors: window.config.pageData.repoWikiActivityTopAuthors || [],
  }),
  computed: {
    graphPoints() {
...
export function initRepoWikiActivityTopAuthorsChart() {
  const el = document.getElementById('repo-wiki-activity-top-authors-chart');
  if (el) {
